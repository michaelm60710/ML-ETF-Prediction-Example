...
def Score(predict, label):
    ...
    one = np.ones((label.shape[0], label.shape[1]), dtype=np.int16)
    no_predict = ((label - np.absolute(one - label))/label)*0.5
    cal = ((label - np.absolute(predict - label))/label)*0.5
    mean_1 = np.mean(no_predict, axis=0)
    mean = np.mean(cal, axis=0)
    print('No predict  ', mean_1, mean_1.sum()/5)
    print('With predict', mean, mean.sum()/5)
    return mean
...
def Run_price_1():
	...
	feature['k/d'] = feature['slowk'] / feature['slowd']
	...
	#Gen label np # Note: 最後一個 label 可能是未來(NaN)
